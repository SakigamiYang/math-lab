...
import numpy as np
from mathlab.numerics.intrinsic_dim import eigengap_dimension_ignore_padding
...
def test_ignore_padding_zeros_does_not_force_d_hat_to_effective_dim():
    ...
    out = eigengap_dimension_ignore_padding(padded)
    assert out["effective_dim"] == 3
    assert out["d_hat"] == 2


def test_all_zero_or_single_positive_returns_d_hat_0():
    out1 = eigengap_dimension_ignore_padding(np.zeros(10))
    assert out1["d_hat"] == 0

    out2 = eigengap_dimension_ignore_padding(np.array([1.0] + [0.0] * 9))
    assert out2["d_hat"] == 0
